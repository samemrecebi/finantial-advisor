...
function ChatPage() {
  ...
  const startNewChat = () => {
    setMessages([]); // Clear the chat messages to start a new chat
  };
    ...
    <div className="flex flex-col h-screen bg-blue-100 font-sans">
      ...
      <div className="flex items-center justify-between p-4">
        <div className="flex items-center space-x-4">
          <Image src="/azure-logo.png" alt="Azure Logo" width={50} height={25} />
          <Image src="/openai-logo.png" alt="OpenAI Logo" width={50} height={25} />
          <button
            type="button"
      ...
      <div className="flex-1 p-4 overflow-y-auto flex flex-col items-center">
        {messages.length === 0 ? (
          ...
          messages.map((message, index) => (
            <div
              key={index}
              className={`flex items-start mb-4 ${message.isUser ? 'justify-end' : 'justify-start'} w-full max-w-3xl`}
            >
              {!message.isUser && (
                <FontAwesomeIcon
                  icon={faRobot}
