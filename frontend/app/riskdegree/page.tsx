...
const riskdegree: React.FC = () => {
  ...
  const handleCompleteTest = (type: string, risk: number, time: string, money: string) => {
    ...
    else if (selectedType === 'Nitelikli Yatırımcı') riskHesap += 1;

    riskHesap += selectedRisk;

    if (selectedTime === 'birOtuzGun') riskHesap += 4;
    else if (selectedTime === 'birUcAy') riskHesap += 3;
    else if (selectedTime === 'ucAltiAy') riskHesap += 2;
    else if (selectedTime === 'altiAyBirYil') riskHesap += 1;
    else if (selectedTime === 'birYil') riskHesap = riskHesap;

    if (selectedMoney === 'onm') riskHesap += 4;
    else if (selectedMoney === 'bironm') riskHesap += 3;
    else if (selectedMoney === 'yuzbirm') riskHesap += 2;
    else if (selectedMoney === 'onyuz') riskHesap += 1;
    else if (selectedMoney === 'onbineksi') riskHesap = riskHesap;

    if (yatirimHedefi === 'yuksekGetiri') riskHesap += 4;
    else if (yatirimHedefi === 'emeklilik') riskHesap += 3;
    else if (yatirimHedefi === 'uzunVadeBuyume') riskHesap += 2;
    else if (yatirimHedefi === 'duzenliGelir') riskHesap += 1;
    else if (yatirimHedefi === 'sermayeKoruma') riskHesap = riskHesap;

    if (riskGetiriTercihi === 'anaParaTamamenKayip') riskHesap += 4;
    else if (riskGetiriTercihi === 'anaParaKayip') riskHesap += 3;
    else if (riskGetiriTercihi === 'anaParaMiktarKayip') riskHesap += 2;
    else if (riskGetiriTercihi === 'anaParaAzKayip') riskHesap += 1;
    else if (riskGetiriTercihi === 'anaParaKoru') riskHesap = riskHesap;

    if (aniDurum === 'hareketsiz') riskHesap += 4;
    else if (aniDurum === 'fayda') riskHesap += 3;
    else if (aniDurum === 'sabir') riskHesap += 2;
    else if (aniDurum === 'durumDegerlendirmesi') riskHesap += 1;
    ...
    const setStringRisk = (riskHesapTemp: number) => {
      if (riskHesapTemp <= 12) return 'Çok Düşük Riskli';
      if (riskHesapTemp > 12 && riskHesapTemp <= 23) return 'Düşük Riskli';
      if (riskHesapTemp > 23 && riskHesapTemp <= 35) return 'Orta Riskli';
      if (riskHesapTemp > 35 && riskHesapTemp <= 47) return 'Yüksek Riskli';
      if (riskHesapTemp > 47) return 'Çok Yüksek Riskli';
    };
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        const response1 = await fetch('/api/users/update', {
          method: 'PUT',
          headers: {
          ...
          body: JSON.stringify({
            ...
            risk_degree: setStringRisk(riskHesap),
          }),
        });

        if (response1.ok) {
          alert('Risk dereceniz başarıyla güncellendi');
        } else {
          alert('Risk derecesi güncellenirken bir hata oluştu');
        }
      } catch (error) {
      ...
      >
        ...
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-[#061178] mb-8">
            Tercih ettiğiniz
            {' '}
            <span className="text-red-600">risk seviyesi</span>
            {' '}
            nedir?
          </h1>
            ...
            {[1, 2, 3, 4, 5, 6, 7].map((risk) => (
              ...
              >
                <div
                  ...
                  style={{ height: `${riskSizes[risk].replace('h-', '')}px`, width: '80px', marginTop: `${200 - parseInt(riskSizes[risk].replace('h-', ''))}px` }}
                >
                  <div
          ...
          <div className="text-center mb-8">
            <h2 className="text-xl font-bold text-[#061178] mb-4">
              Risk Değeri
              {' '}
              <span className="text-3xl font-extrabold text-[#061178]">{selectedRisk}</span>
            </h2>
          ...
          <div className="w-full flex items-center justify-between">
            <div className="pl-16">
              <h1
                ...
                style={{
                  right: '-5rem', width: 'fit-content', maxWidth: 'calc(100% - 10rem)', whiteSpace: 'normal',
                }}
              >
          ...
          <div className="w-full flex items-center justify-between">
            <div className="pl-16">
              <h1
                className="text-3xl font-bold text-[#061178] relative"
                style={{
                  right: '-8rem', width: 'fit-content', maxWidth: 'calc(100% - 20rem)', whiteSpace: 'normal',
                }}
              >
